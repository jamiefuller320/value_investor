...
def _cmd_list_parked(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for row in rows:
            item = dict(row)
            hint = stall_hints.get(str(row.get("id") or ""))
            if hint:
                item["library_stall_triage"] = hint
            enriched.append(item)
        _print_json({"parked": enriched, "count": len(enriched)})
    elif not rows:
        print("No parked engineering tasks")
    else:
        for row in rows:
            line = f"{row['id']}: {row.get('parked_reason')}"
            hint = stall_hints.get(str(row.get("id") or ""))
            if hint:
                focus = hint.get("focus_ticker") or "?"
                unpark = hint.get("allow_unpark")
                line += f" | stall focus={focus} allow_unpark={unpark}"
            print(line)
    return 0
...
def _cmd_eng_narrow_gate(args: argparse.Namespace) -> int:
    """CI gate for ingest_narrow / scoring_narrow PRs (deterministic independent verify)."""
    ...
    if task is None:
        payload = {
            "ok": True,
            "verdict": "skipped",
            "reason": "unknown engineering task — skipped",
        }
        if args.json:
            _print_json(payload)
        else:
            print("eng-narrow-gate: skipped (unknown task)")
        return 0
    if not task_is_narrow_candidate(task):
        payload = {
            "ok": True,
            "verdict": "skipped",
            "reason": "not a narrow-merge candidate — skipped",
            "task_id": task.id,
        }
        if args.json:
            _print_json(payload)
        else:
            print("eng-narrow-gate: skipped (not narrow class)")
        return 0
    ...
    gate = evaluate_narrow_verify(task=task, changed_files=changed_files)
    if args.json:
...
def main(argv: list[str] | None = None) -> int:
    ...
    eng_narrow_p = sub.add_parser(
        "eng-narrow-gate",
        parents=[common],
        help=(
            "CI independent-verify gate for narrow ingest/scoring PRs "
            "(actual diff ≤8 safe paths + tests; policy off|observe|merge)"
        ),
    )
    eng_narrow_p.add_argument("--branch", required=True)
    eng_narrow_p.add_argument("--base-ref", default="origin/main")
    eng_narrow_p.add_argument("--head-ref", default="HEAD")
    eng_narrow_p.add_argument(
        "--changed-files",
        default=None,
        help="Newline-delimited changed paths (default: git diff base...head)",
    )
    eng_narrow_p.set_defaults(func=_cmd_eng_narrow_gate)
