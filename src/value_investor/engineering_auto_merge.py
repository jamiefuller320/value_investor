...
def evaluate_auto_merge(
    *,
    ...
    ingest_narrow_candidate = task_is_ingest_narrow_candidate(task)

    if not hunter_task and not ci_fix_eligible and not ingest_narrow_candidate:
        return AutoMergeDecision(
            False,
    ...
    allowed, narrow_reason = ingest_narrow_merge_allowed(task=task, changed_files=changed)
    if allowed:
        return AutoMergeDecision(
            True,
            narrow_reason,
