#!/usr/bin/env python3
"""Research remaining quality buy-tier names on the S&P library screen."""
...
from value_investor.data_quality import MIN_QUALITY_FOR_BUY
...
LOG = SCREEN / "research_buy_tier_finish.json"
...
def main() -> int:
    ...
    if not remaining:
        print("No remaining quality buys without memos.", flush=True)
        return 0

    before_cycle = float((policy.get("budget") or {}).get("estimated_spend_usd_this_cycle") or 0.0)
    log: dict = {
        "test": "sp500_finish_buy_tier",
    ...
    print(f"Finishing {len(remaining)} buy-tier memos with {model}", flush=True)
