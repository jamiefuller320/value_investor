...
| `project-traffic.yml` | External **primary** | Weekdays `30 12 * * 1-5` + `30 17 * * 1-5` → `project-traffic.yml` | Weekdays 12:30 + 17:30 |
...
| `dashboard-bridge.yml` | External **primary** | Every day every **10m** (`0/10/20/30/40/50` × all hours; `wdays` `[-1]`) → `dashboard-bridge.yml` | `*/10 * * * *` (backup; often drifts hours) |
