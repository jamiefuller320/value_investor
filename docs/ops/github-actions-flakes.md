...
| Sunday `email-report` cancelled at ~6h mid PDF/OCR | Full buy-tier deepen inside quiet bundle | Saturday pre-Sunday `ingest-loop` deepen; Sunday email skips `--ingest-improvement-pass` ([ingest-scan-then-target.md](ingest-scan-then-target.md#saturday-pre-sunday-deepen)) |
