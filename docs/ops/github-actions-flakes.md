...
| Sunday `email-report` cancelled at ~6h mid PDF/OCR (`incorrect startxref` / Object Streams) | Ingest-improvement ran **without** `max_runtime_seconds`, so cutoff never fired and CH deepen OCR burned the job; Send/commit skipped | `email-report.yml` passes `--ingest-max-runtime-seconds 7200`, sets `timeout-minutes: 360`, and caps `COMPANIES_HOUSE_DEEPEN_OCR_MAX_PAGES` (L429) |
...
| Library epoch-0 weekday — *Commit epoch-0 weekday artifacts* rejected (`cannot lock ref` / `fetch first`) | Marks (or census-only timestamp churn) already ran; `git-auto-commit` lost a race with engineering-queue / ingest on `main` | `scripts/gha_commit_artifacts.sh` in `library-epoch0-weekday.yml`. Check job JSON: `marked: 0` + `already_marked_local_day` means books are fine |
