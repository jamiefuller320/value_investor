...
**Equal-support is the on-ramp to future per-market AI / autopsy (data only).** L321 / L322 already guarantee every *admitted* market the same opportunity to accumulate the ingredients later shard AI eligibility and **L370** freeze snapshots will need: weekday epoch-0 marks, near-miss watch (buy-not-now / hold-near-buy), buy-tier rememo eligibility, Layer B archives, and FTSE-volume filing maintenance. Keep that *accumulation* balanced across the admitted set. Deliberate *opening* / cascade skews are not permission to starve another admitted book’s data path: euro fat-sprint volume (serialize to threshold), FTSE P1 live depth, capacity-1 weekly paper (**N94**), and shared rememo daily caps. Parallel Phase C attribution and shard AI-judgment stay serial (**N122**); parallel equal-support *data* stays on. Outcome counts will still differ by jurisdiction yield and buy-tier width — that is residual, not unequal wiring.

**What still waits.** Shard AI-judgment and knob apply wait on the epoch-0 + near-miss watch. Phase 2 weekly AI paper stays on `euro_depth` only. FTSE remains the P1 data lead. Equal decision-freeze fields on admitted epoch-0 rows (**L370**) wait on a proven FTSE freeze schema — that thickens data for later AI timing; it is not opening AI.
...
- Non-UK shards also cold-start **`buy_tier_level_native`** (N153): same policy, market-native reporting currency, no FX friction. The GBP `buy_tier_level` book is kept as an FX-warped archive — do not rewrite it mid-flight or back-label its NAV. See [`buy-tier-cohort-labs.md`](buy-tier-cohort-labs.md#native-currency-twin-for-non-uk-shards-buy_tier_level_native).
...
`library-epoch0-weekday.yml` marks admitted `buy_tier_level` books after each market's open+settle (ASX 00:45, EU 08:45, US 14:15/15:15 UTC). It does **not** dispatch FTSE `paper-auto.yml`. If buy-tier timing is still unresolved it stamps before the mark (same wait filter as Sunday). After a mark it refreshes `equal_support_status.json` with `--census-only`. Artifact commits use `scripts/gha_commit_artifacts.sh` (fetch + overlay + push retry) so a concurrent `main` writer (engineering-queue, ingest) does not fail the job after marks already ran.
...
| **One maintenance job, many markets** | Two markets fit; width defaults to **2** when crowded (L454) | Width 3+ without capacity-review green samples — job now rotates a bounded set per slot (L323) instead of clipping the tail |

**Spare auto-advance is correct.** When a spare stream hits `sprint_ingest_complete`, promote the next `market_queue` name into that slot (`tsx60` / `ftse_smallcap` today). Do not pause that rotation to “save capacity.” Shared `library-ingest-maintenance` with three-plus books serves **up to two markets per cron** at full FTSE volume by default (`maintenance_capacity.json` + `maintenance_slot_cursor.json`; L323/L454). `ftse-library maintenance-capacity-review --apply` steps width 1–3 from slot samples or proposes matrix (flag only). Still watch job finish: `timeout-minutes: 120`, spare_wait, ESEF/EDGAR/IR/Yahoo rate limits. Do not add a fourth equal sprint stream. The per-market **62-name** deepen cap is usually not binding on maintenance books.
