...
**Cron registration (automated on admit).** First-time `admit_market_to_learning` upserts the timezone-bucket cron-job.org jobs for that market's session (`asx` / `euro` / `us-edt`+`us-est`). The same ensure runs on `ftse-library euro-ingest-dispatch --refresh --sync-cron` for the full admitted roster (catch-up). Requires `CRONJOB_API_KEY` + `WORKFLOW_DISPATCH_PAT` on ingest/sprint/maintenance workflows. Soft-skips when secrets are missing. Residual human only when admitting a market whose session timezone has **no** mapped bucket — extend `EPOCH0_WEEKDAY_SLOTS` in `epoch0_weekday_cron.py`, then re-run sync.
