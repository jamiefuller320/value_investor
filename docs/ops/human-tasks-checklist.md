...
| Review **entry DCA cadence / adoption plan** — ack is recorded (observe-only; CLI or Lifecycle **Start** via Supabase); follow `entry_dca_adoption_plan.json` gates; do **not** execute DCA, change starter fraction, or apply to primary | Human | [position-lifecycle.md](position-lifecycle.md#entry-dca-adoption-plan) |
