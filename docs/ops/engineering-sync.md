...
| **Parallel dispatch** | `max_parallel_engineering_agents` (default 2) with **clash-aware** selection — walks priority order, skips tasks blocked by allowlist overlap, open-PR file overlap, shared mutable files (`engineering_tasks.json`, `policy.json`, …), or optional `git merge-tree` conflicts; non-competing tasks **overtake** blocked higher-priority work; after the first pick, prefers a complementary `area` (e.g. scoring next to ingest) among still-eligible tasks |
