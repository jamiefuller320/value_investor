...
Collated from accessible Cursor Cloud agent sessions (as of 2026-07-16).

**Scope note:** This environment can only see **5** agent runs for the repo owner. Many historical PR branches exist (`cursor/*`), but their transcripts were not fetchable here. Re-run this collation after more agents are visible, or paste additional session URLs to extend the list.

**How to use:** Review quarterly (or after ~8–12 weekly archives). Move items to “done”, “drop”, or “now” as evidence accumulates.
...
| N2 | **Evolutionary / survival-of-the-fittest parallel sims** | Sparse weekly history → overfitting; high-churn genomes look fit until costs dominate | Many tens of weekly snapshots + cost-penalised walk-forward fitness |
...
| L5 | **Trade-plan magic numbers → config** | Hard-coded thresholds fine until tuning/backtests | Tuning trade-plan params |
...
| # | Idea | Summary | Revisit when |
|---|------|---------|--------------|
| L14 | **Research beyond capped buy tier** / short-adversarial mode | Don’t auto-research holds/avoids yet | Capped buy-tier memos prove decision value |
...
| # | Idea | Summary | Revisit when |
|---|------|---------|--------------|
| L21 | **External cron → `workflow_dispatch`** | Fallback if GitHub scheduler stays silent | Monday `schedule` misses again |
...
## Security / hygiene follow-ups (time-sensitive, not features)

| # | Item | Note |
|---|------|------|
| S1 | **Rotate any API/SMTP secrets** that were ever committed or pasted in chat | Git history may still contain old keys even after `.gitignore` fixes |
| S2 | **Re-verify `CURSOR_API_KEY` in a fresh Cloud Agent** | Secrets may not inject into an already-running pod |
| S3 | **Quote special characters in `.env`** | Unquoted passwords break `source .env` |

---

## Suggested review cadence

1. **After each ~4 weekly archives:** check L1 (decision-review), L6, L22.
2. **After ~8–12 weeks / “many tens” of runs:** re-open N2/L2 (evolution) and N1/L7 (universe).
3. **When runtime or data quality hurts:** L11, L23, L13.
4. **When acting on the buy list feels underspecified:** L3, L19.

---

## Still-missing session coverage

Remote branches suggest older agents not listed in this environment (examples): historical analysis, portfolio simulator, research filings, technical timing, risk/adaptive weights, FTSE 250 universe, etc. Their deferred advice is **not** in this file yet.
