# Buy-tier chart outcomes

Mixed story — no terrible outcomes. 16 well timed (including MEGP.L (+10.1%), AEP.L (+8.0%), GFTU.L (+6.4%)); 7 target-then-fade; 24 underwater; 0 stop hits.

**Verdict:** Mixed story — no terrible outcomes (`mixed_no_terrible`)

Observe-only. Frozen initial recommendation levels + first crossings. Not a decision-review input.

Short-term underwater is expected while the hypothesis stands. The test is the longer path — do not retune entry timing from this mix.

## Counts

- Charts: 59
- Well timed: 16
- Target then fade: 7
- Underwater (no target): 24
- Intact positive: 7
- Flat: 5
- Terrible: 0
- Stop hits: 0
- Target hits: 23

## Returns since recommendation

- Median: -0.2%
- Mean: -0.6%
- Range: -8.9% to +10.1%
- Median drawdown: -2.4%
- Worst drawdown: -14.7%

## Well timed

| Ticker | Signal | Return | Drawdown | Days to target |
|---|---|---:|---:|---:|
| MEGP.L | strong_buy | +10.1% | +5.6% | 1 |
| AEP.L | strong_buy | +8.0% | -3.2% | 17 |
| GFTU.L | strong_buy | +6.4% | +3.5% | 2 |
| RS1.L | buy | +5.5% | +2.1% | 3 |
| PTEC.L | buy | +4.5% | -4.0% | 22 |
| FRES.L | buy | +4.2% | -0.1% | 3 |
| BYIT.L | buy | +3.8% | +0.4% | 24 |
| MGAM.L | buy | +3.5% | +2.9% | 3 |

## Weakest open returns

| Ticker | Signal | Return | Drawdown | Outcome | Stop | Target |
|---|---|---:|---:|---|---|---|
| JD.L | buy | -8.9% | -10.5% | giveback | no | yes |
| TRN.L | buy | -8.3% | -9.0% | underwater | no | no |
| TPK.L | buy | -8.0% | -8.0% | underwater | no | no |
| FOUR.L | buy | -7.7% | -7.7% | underwater | no | no |
| SBRY.L | buy | -6.4% | -8.5% | underwater | no | no |
| ITV.L | strong_buy | -6.1% | -6.1% | underwater | no | no |
| WIX.L | buy | -5.7% | -5.7% | underwater | no | no |
| SN.L | strong_buy | -5.3% | -5.3% | underwater | no | no |

_Observe-only rollup of buy-tier chart JSON. Entry is the frozen initial last (recommendation-week close), not the first bar after signal_since. Short-term underwater is expected while the hypothesis stands — the test is the longer path. Do not apply decision-review knobs or entry-timing overlays from this file._

