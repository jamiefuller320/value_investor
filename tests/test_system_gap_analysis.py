...
def test_snapshot_flags_admitted_rememo_when_weekday_escalate(tmp_path: Path):
    data = tmp_path / "data"
    library = tmp_path / "library"
    _write_json(
        library / "last_ladder.json",
        {
            ...
            "layers": {
                "selective_research": {
                    "executed": 1,
                    "allow_research": True,
                    "research_markets": ["euro_depth", "sp500", "asx200"],
    ...
    _write_json(
        library / "admitted_rememo_backlog.json",
        {
            "eligible_total": 61,
            "markets": {
                "sp500": {"eligible_count": 54, "action": "escalate"},
                "asx200": {"eligible_count": 7, "action": "maintain"},
            },
        },
    )
    _write_json(
...
def test_snapshot_does_not_flag_admitted_rememo_on_weekday_path(tmp_path: Path):
    data = tmp_path / "data"
    library = tmp_path / "library"
    _write_json(
        library / "equal_support_status.json",
        {
            "admitted": ["sp500"],
            "markets": {"sp500": {"rememo_eligible_count": 54}},
        },
    )
    _write_json(
        library / "admitted_rememo_backlog.json",
        {
            "eligible_total": 54,
            "markets": {"sp500": {"eligible_count": 54, "action": "catch_up"}},
        },
    )
    _write_json(
        library / "policy.json",
    ...
    ids = {row["id"] for row in snapshot["flags"]}
    assert "admitted_rememo_not_queued" not in ids
...
def test_observe_clock_skips_sprint_markets_without_benchmark(tmp_path: Path):
    data = tmp_path / "data"
    library = tmp_path / "library"
    screen = library / "markets" / "aim" / "screen"
    screen.mkdir(parents=True)
    ...
    assert "aim" not in clock_ids


def test_observe_clock_includes_ftse_smallcap_sprint(tmp_path: Path):
    data = tmp_path / "data"
    library = tmp_path / "library"
    ...
    clock_ids = {row["market_id"] for row in snapshot["layers"]["learning_clock"]["markets"]}
    assert "ftse_smallcap" in clock_ids
