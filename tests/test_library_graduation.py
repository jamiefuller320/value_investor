...
def test_research_markets_includes_full_queue_before_graduation():
    ...
    policy = {
        ...
        "ladder": {"research_all_graduated": True},
    }
...
def test_sunday_rememo_reasons_stay_on_focus_when_depth_first():
    ...
    assert sunday_rememo_reasons(
        reasons, focus_tickers=focus, research_all_graduated=False
    ) == {"ERIC-B.ST": "lag_10"}
    assert sunday_rememo_reasons(
        reasons, focus_tickers=focus, research_all_graduated=True
    ) == reasons
