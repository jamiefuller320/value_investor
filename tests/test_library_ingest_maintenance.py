...
def test_list_library_ingest_maintenance_markets_excludes_sprinting_focus_even_if_admitted():
    policy = {
        "focus_market": "euro_depth",
        "ladder": {"admitted_learning_markets": ["euro_depth", "asx200"]},
    }
    gappy = {
        "unmeasured_buy_tier": 1,
        "zero_body_buy_tier": 0,
        "thin_body_buy_tier": 2,
    ...
    with patch(
        ...
        markets = list_library_ingest_maintenance_markets(policy=policy)
    assert markets == ["asx200"]


def test_list_library_ingest_maintenance_markets_keeps_admitted_when_parity_dips():
    """Learning markets keep the deepen loop so newly screened names stay in view."""
    policy = {
        "focus_market": "euro_depth",
...
def test_run_library_ingest_maintenance_deepens_admitted_when_parity_dips():
    admitted_health = {
        "unmeasured_buy_tier": 0,
        "zero_body_buy_tier": 0,
        "thin_body_buy_tier": 2,
    ...
    policy = {
        "focus_market": "euro_depth",
        "ladder": {"admitted_learning_markets": ["asx200"]},
    }
    with (
        patch(
            "value_investor.library_ingest_maintenance.list_library_ingest_maintenance_markets",
