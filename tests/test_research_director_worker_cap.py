...
def test_exploration_cap_allows_runs_until_limit(tmp_path: Path):
    ...
    first = check_director_worker_cap(
        "AAA.L", policy_path=policy_path, ledger_path=ledger_path, when=when
    )
    ...
    second = check_director_worker_cap(
        "BBB.L", policy_path=policy_path, ledger_path=ledger_path, when=when
    )
    ...
    blocked = check_director_worker_cap(
        "CCC.L", policy_path=policy_path, ledger_path=ledger_path, when=when
    )
...
def test_reescalation_detected_on_repeat_ticker(tmp_path: Path):
    ...
    status = check_director_worker_cap(
        "VTY.L", policy_path=policy_path, ledger_path=ledger_path, when=when
    )
...
def test_auto_tighten_moves_to_steady_after_stable_window(tmp_path: Path):
    ...
    status = check_director_worker_cap(
        "NEW.L", policy_path=policy_path, ledger_path=ledger_path, when=base
    )
...
def test_auto_tighten_skipped_when_reescalation_rate_high(tmp_path: Path):
    ...
    assert (
        tighten["reason"] == "insufficient_weeks" or tighten["reason"] == "reescalation_rate_high"
    )
