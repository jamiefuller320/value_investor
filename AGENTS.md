...
2. **P2 — Offline ingest cascade.** Serialize one market at a time to the **maintenance ingest threshold** (`sprint_ingest_complete`: raw parity or leftover thin/IWB parked). Fat slot on that head; spare streams only front-start the next queue names. When the head graduates, shift the fat slot and give the graduated market **equivalent resource** (FTSE-volume maintenance + the same learning package) — do not leave it on leftover observe-sim (L321 / L322). Do **not** add a fourth equal sprint stream that can starve the current head. See [`docs/ops/market-sharded-learning.md`](docs/ops/market-sharded-learning.md#what-enter-learning-means).
